...
class DVHBand:
    ...
    def compute_metrics(self):
        # compute metrics
        self.D98 = self.computeBandDx(98)
        self.D95 = self.computeBandDx(95)
        self.D50 = self.computeBandDx(50)
        self.D5 = self.computeBandDx(5)
        self.D2 = self.computeBandDx(2)

    def computeBandDx(self, x):
        index = np.searchsorted(-self.volumeLow, -x)
        if index > len(self.volumeLow) - 2: index = len(self.volumeLow) - 2
        volume = self.volumeLow[index]
        volume2 = self.volumeLow[index + 1]
        if volume == volume2:
        ...
        index = np.searchsorted(-self.volumeHigh, -x)
        if index > len(self.volumeHigh) - 2: index = len(self.volumeHigh) - 2
        volume = self.volumeHigh[index]
        volume2 = self.volumeHigh[index + 1]
        if volume == volume2:
