...
def signalGeneration(amplitude=10, dA=5, period=4.0, df=0.5, dS=5, mean=0, sigma=1, step=0.2, signalDuration=100, L=2 / 30):
    ...
    """
    Ya = vectorSimulation(L, dA, mean, sigma, signalDuration, timestamps,listOfEvents)
    amplitude += Ya
    s = vectorSimulation(L, dS, mean, sigma, signalDuration, timestamps,listOfEvents)
    Yf = vectorSimulation(L, df, mean, sigma, signalDuration, timestamps,listOfEvents)
    freq += Yf
    
    plt.figure(figsize=(15,10))
    plt.plot(timestamps,Ya)
    plt.title("Amplitude")
    
    plt.figure(figsize=(15,10))
    plt.plot(timestamps,Yf)
    plt.title("Frequency")
