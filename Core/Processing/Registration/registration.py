...
class Registration:
    ...
    def regularizeField(self, field, filterType="Gaussian", sigma=1.0, cert=None, tryGPU=True):
        ...
        if filterType == "Gaussian":
            field.velocity._imageArray[:, :, :, 0] = imageFilter3D.gaussConv(field.velocity._imageArray[:, :, :, 0], sigma=sigma, tryGPU=tryGPU)
            field.velocity._imageArray[:, :, :, 1] = imageFilter3D.gaussConv(field.velocity._imageArray[:, :, :, 1], sigma=sigma, tryGPU=tryGPU)
            field.velocity._imageArray[:, :, :, 2] = imageFilter3D.gaussConv(field.velocity._imageArray[:, :, :, 2], sigma=sigma, tryGPU=tryGPU)
            return

        if filterType == "NormalizedGaussian":
            if cert is None:
                cert = np.ones_like(field.velocity._imageArray[:, :, :, 0])
            field.velocity._imageArray[:, :, :, 0] = imageFilter3D.normGaussConv(field.velocity._imageArray[:, :, :, 0], cert, sigma, tryGPU=tryGPU)
            field.velocity._imageArray[:, :, :, 1] = imageFilter3D.normGaussConv(field.velocity._imageArray[:, :, :, 1], cert, sigma, tryGPU=tryGPU)
            field.velocity._imageArray[:, :, :, 2] = imageFilter3D.normGaussConv(field.velocity._imageArray[:, :, :, 2], cert, sigma, tryGPU=tryGPU)
            return
