...
exportImageSitk(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', experimentName,'phantom.nrrd'), phantom)
...
array = gaussian_filter(array, sigma=3)
targetMask = ROIMask('target')
targetMask.imageArray = array 
targetMask.spacing = spacing_mm
targetMask.origin = Origin
exportImageSitk(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', experimentName, 'mask.nrrd'), targetMask)

targetMaskProb = ProbabilityMap(targetMask)

planInit = PhotonPlanDesign()
planInit.ct = phantom
planInit.targetMask = targetMaskProb.thresholdProbMap(0.01)
...
planInit.robustness.modality = RobustnessPhotons.Modality.PRO
plan = planInit.buildPlan() 

DIM_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', experimentName, 'SparseMatrices_MinMax')
if not os.path.isdir(DIM_path) or len(os.listdir(DIM_path)) == 0:
    ...
    saveDIM(plan, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', experimentName), 'MinMax', 'all')
else:
    loadDIM(plan, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', experimentName, 'SparseMatrices_MinMax'))

plan.planDesign.objectives = ObjectivesList()
plan.planDesign.objectives.setTarget('target', 70)
plan.planDesign.objectives.addFidObjective(targetMaskProb, 'DMax', 70, 10, robust = True)
plan.planDesign.objectives.addFidObjective(targetMaskProb, 'DMin', 70.5, 10, robust = True)
plan.planDesign.objectives.addFidObjective(phantomMask, 'DMax', 0, 1, robust = False)
...
dose_file_nrrd = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', experimentName, "dose.nrrd") 
