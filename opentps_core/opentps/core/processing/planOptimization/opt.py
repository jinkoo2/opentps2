...
class PlanOptimizer:
    ...
    def initializeWeights(self):
        ...
        if self.opti_params.get('init_weights') is not None:
            x0 = self.opti_params['init_weights']
            if self.xSquared:
                x0 = np.sqrt(x0)
        else:
            self.plan.planDesign.beamlets.beamletWeights = np.ones(len(self.plan.planDesign.beamlets.beamletWeights))
    ...
    def postProcess(self, result):
        ...
        if self.plan.planDesign.robustness is not None:
            self.plan.planDesign.robustness.setBeamWeights(self.plan.spotMUs)
