...
class FullyProbabilisticDoseFidelity(BaseFunc):
    ...
    def computeFidelityFunction(self, x, returnDiff = False):
        if self.xSquare:
            weights = np.square(x).astype(np.float32)
        else:
            weights = x.astype(np.float32)
        ...
        scenarioList = []
        fTotNominalNoRO = 0.0
        beamlets = [self.beamlets] + self.scenariosBL

        # Compute objectives for error scenarios
        for i, ScenarioBL in enumerate(beamlets):
            fTotScenario = 0.0
            ...
            for objective in self.list:
                if not objective.robust and i != 0:
                    continue

                if objective.metric == objective.Metrics.DMAX:
                    f = np.sum(objective.ProbMap * np.maximum(0, doseTotal[objective.maskVec] - objective.limitValue) ** 2) / np.sum(objective.ProbMap)
                elif objective.metric == objective.Metrics.DMEAN:
                    f = np.maximum(0,
                                   np.sum(objective.ProbMap * doseTotal[objective.maskVec], dtype=np.float32) /  np.sum(objective.ProbMap) - objective.limitValue) ** 2
                elif objective.metric == objective.Metrics.DMIN:
                    f = np.sum(objective.ProbMap * np.minimum(0, doseTotal[objective.maskVec] - objective.limitValue) ** 2) / np.sum(objective.ProbMap)
                else:
                    raise Exception(objective.metric + ' is not supported as an objective metric')

                if i == 0 and not objective.robust:
                    fTotNominalNoRO += objective.weight * f
                else:
                    fTotScenario += objective.weight * f
        ...
        mean = np.sum(scenarioList * self.prob)
        self.diff = scenarioList - mean
        deviation = np.sqrt(np.sum((self.diff)**2 * self.prob))
        fTotProb = (1-self.epsilon) * mean + self.epsilon * deviation
        if returnDiff:
            return  fTotProb + fTotNominalNoRO, self.diff
        else:
            return fTotProb + fTotNominalNoRO


    def computeFidelityGradient(self, x):
        # get worst case scenario
        # _, diff = self.computeFidelityFunction(x, returnDiff = True)
        diff = self.diff
        gradientScenarios = []
        dfNominalNoRO = 0.0
        xDiag = sp.diags(x.astype(np.float32), format='csc')
        ...
        for i, ScenarioBL in enumerate(beamlets):
            ...
            if use_MKL == 1:
                ...
                doseTotal = sparse_dot_mkl.dot_product_mkl(ScenarioBL, weights)
                if self.xSquare:
                    doseBL = sparse_dot_mkl.dot_product_mkl(ScenarioBL, xDiag)
                else:
                    doseBL = ScenarioBL
            else:
                dfTot = np.zeros((len(x), 1), dtype=np.float32)
                doseTotal = sp.csc_matrix.dot(ScenarioBL, weights)
                if self.xSquare:
                    doseBL = sp.csc_matrix.dot(ScenarioBL, xDiag)
                else:
                    doseBL = self.beamlets
                doseBL = sp.csc_matrix.transpose(doseBL)

            for objective in self.list:
                if not objective.robust and i != 0:
                    continue
                if objective.metric == objective.Metrics.DMAX:
                    f = objective.ProbMap * np.maximum(0, doseTotal[objective.maskVec] - objective.limitValue) / np.sum(objective.ProbMap)
                    if use_MKL == 1:
                        f = sp.diags(f.astype(np.float32), format='csc')
                        df = sparse_dot_mkl.dot_product_mkl(f, doseBL[objective.maskVec, :])
                    else:
                    ...
                    f = np.maximum(0, np.sum(objective.ProbMap * doseTotal[objective.maskVec], dtype=np.float32) / np.sum(objective.ProbMap) - objective.limitValue) * objective.ProbMap / np.sum(objective.ProbMap)
                    if use_MKL == 1:
                        df = sp.csr_matrix.multiply(doseBL[objective.maskVec, :], f)
                    else:
                        df = sp.csr_matrix.multiply(doseBL[:, objective.maskVec], f)
                    ...
                    f = objective.ProbMap * np.minimum(0, doseTotal[objective.maskVec] - objective.limitValue) / np.sum(objective.ProbMap)
                    if use_MKL == 1:
                        f = sp.diags(f.astype(np.float32), format='csc')
                        df = sparse_dot_mkl.dot_product_mkl(f, doseBL[objective.maskVec, :])
                    else:
                ...
                if use_MKL == 1:
                    df = objective.weight * df.sum(axis=0)
                else:
                    df = objective.weight * df.sum(axis=1)
            ...
            gradientScenarios.append(dfTot)
        

        if self.xSquare:
            dfNominalNoRO = 4 * dfNominalNoRO
        else:
            dfNominalNoRO = 2 * dfNominalNoRO

        gradientMatrix = np.squeeze(np.array(gradientScenarios))
        probMatrix = self.prob.reshape((1,len(self.prob)))
        diffMatrix = diff.reshape((1,len(diff)))
        diffMean = probMatrix.dot(gradientMatrix)
        dfTotScenario =  diffMean * (1 - self.epsilon) + self.epsilon * 1/np.sqrt(np.sum(probMatrix * diffMatrix **2)) * (probMatrix * diffMatrix).dot(gradientMatrix - diffMean)
        dfTotal = dfTotScenario + np.transpose(dfNominalNoRO)
        return np.squeeze(np.asarray(dfTotal)).astype(np.float64)
